...
import { PROVIDERS, MODELS } from './types'
import './App.css'
...
function App() {
  ...
  useEffect(() => {
    ...
    if (match) {
      const token = decodeURIComponent(match[1].replace(/&.*$/, ''))
  ...
  const applyPreset = (preset: Preset) => {
    ...
    localStorage.setItem(PRESETS_KEY + '-active', preset.id)
  ...
  const saveCurrentAsPreset = async () => {
    ...
    try {
      ...
      localStorage.setItem(PRESETS_KEY + '-active', newPreset.id)
    } catch (e) {
      setValidationErrors([(e as Error).message])
    } finally {
      setValidating(false)
  ...
  const deletePreset = (id: string) => {
    ...
    if (activePresetId === id) {
      ...
      else localStorage.removeItem(PRESETS_KEY + '-active')
    }
  }
      ...
      <aside className={`sidebar ${sidebarOpen ? 'open' : ''}`}>
        ...
        <div className="sidebar-body">
          {authLoading ? (
            ...
            <>
              <p className="signed-in-as">Signed in as <strong>{user.email}</strong></p>
          ...
          ) : sessionToken && !serverSettings?.hasApiKey ? (
            <>
